...
if __name__ == "__main__":
    ...
    # Graphique du domaine frequentiel du son de guitare
    plt.figure("Fft du signal d'entrée")
    plt.title("Spectre fréquentiel du LA# de la guitare")
    plt.ylabel("Amplitude (dB)")
    plt.xlabel("Frequency (Hz)")
    plt.xlim(0,22050)
    plt.plot(np.arange(-sr/2,sr/2),20*np.log10(np.fft.fftshift(np.abs(X))))
    ...
    plt.ylabel('Amplitude (dB)')
    plt.plot(nf*sr/Nf,20*np.log10(hf))
